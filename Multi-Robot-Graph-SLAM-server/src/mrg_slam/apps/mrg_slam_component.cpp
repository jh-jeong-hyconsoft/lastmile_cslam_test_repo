...
public:
    ...
    virtual void onInit()
    {
        ...
        // B-Plan: Check if this is server backend mode (receives keyframes) or robot mode (processes sensor data)
        enable_keyframe_event_reception_ = get_parameter( "enable_keyframe_event_reception" ).as_bool();

        if( enable_keyframe_event_reception_ ) {
            // Server Backend Mode: Subscribe to keyframe_upload from robots
            RCLCPP_INFO( get_logger(), "B-Plan Server Mode: Subscribing to slam/keyframe_upload" );
            keyframe_upload_sub_ = create_subscription<mrg_slam_msgs::msg::KeyframeEvent>(
                "slam/keyframe_upload", 
                rclcpp::QoS( 10 ).reliable(),
                std::bind( &MrgSlamComponent::keyframe_upload_callback, this, std::placeholders::_1 ) );
        } else {
            // Robot Mode: Subscribe to sensor data
            RCLCPP_INFO( get_logger(), "Robot Mode: Subscribing to sensor data (odom + cloud)" );
        ...
        // Map points publishers with Transient Local QoS (for RViz compatibility)
        map_points_timer_pub_   = create_publisher<sensor_msgs::msg::PointCloud2>( "mrg_slam/map_points", 
                                                                                   rclcpp::QoS( 1 ).transient_local() );
        map_points_service_pub_ = create_publisher<sensor_msgs::msg::PointCloud2>( "mrg_slam/map_points_service", 
                                                                                   rclcpp::QoS( 1 ).transient_local() );
        ...
        keyframe_event_pub_ = create_publisher<mrg_slam_msgs::msg::KeyframeEvent>( "slam/keyframe_event", rclcpp::QoS( 10 ) );
...
private:
    void initialize_params()
    {
        ...
        // B-plan: Keyframe event publishing (for server upload)
        declare_parameter<bool>( "enable_keyframe_event_publish", false );
        declare_parameter<double>( "keyframe_event_downsample_resolution", 0.5 );
        // B-plan: Keyframe event reception (server backend mode)
        declare_parameter<bool>( "enable_keyframe_event_reception", false );
    ...
    void cloud_callback( nav_msgs::msg::Odometry::ConstSharedPtr odom_msg, sensor_msgs::msg::PointCloud2::ConstSharedPtr cloud_msg )
    {
        ...
        if( update_required ) {
            ...
            // B-plan: Publish keyframe event for server upload
            if( get_parameter( "enable_keyframe_event_publish" ).as_bool() ) {
                publish_keyframe_event( stamp, odom, accum_d, cloud );
            }
        }
    ...
    void publish_keyframe_event( const rclcpp::Time &stamp, const Eigen::Isometry3d &odom, double accum_d, 
                                 pcl::PointCloud<PointT>::Ptr cloud )
    {
        // B-plan: Publish keyframe event for server upload with additional downsampling
        mrg_slam_msgs::msg::KeyframeEvent kf_event;
        kf_event.header.stamp    = stamp;
        ...
        pcl::PointCloud<PointT>::Ptr ds_cloud( new pcl::PointCloud<PointT>() );
        
        pcl::ApproximateVoxelGrid<PointT> voxelgrid;
        voxelgrid.setLeafSize( ds_resolution, ds_resolution, ds_resolution );
        voxelgrid.setInputCloud( cloud );
        voxelgrid.filter( *ds_cloud );
        
        pcl::toROSMsg( *ds_cloud, kf_event.cloud );
        kf_event.cloud.header.stamp    = stamp;
        kf_event.cloud.header.frame_id = base_frame_id_;
    ...
    /**
     * @brief B-Plan: Callback for receiving keyframe_upload from robots (server backend mode)
     * @param msg KeyframeEvent message from robot
     */
    void keyframe_upload_callback( const mrg_slam_msgs::msg::KeyframeEvent::SharedPtr msg )
    {
        std::unique_lock<std::mutex> unique_lck( main_thread_mutex_ );
        
        RCLCPP_INFO( get_logger(), "Received keyframe upload: robot=%s, uuid=%s, accum_dist=%.2f, cloud_size=%zu",
                    msg->robot_name.c_str(), msg->keyframe_uuid.c_str(), msg->accum_distance, msg->cloud.data.size() );
        
        // Validation
        if( msg->robot_name.empty() ) {
            RCLCPP_WARN( get_logger(), "Received keyframe with empty robot_name, skipping" );
            return;
        }
        
        if( msg->cloud.data.empty() ) {
            RCLCPP_WARN( get_logger(), "Received keyframe with empty cloud, skipping" );
            return;
        }
        ...
        if( cloud->empty() ) {
            RCLCPP_WARN( get_logger(), "Converted cloud is empty, skipping" );
            return;
        }
        
        // Convert pose to Eigen::Isometry3d
        Eigen::Isometry3d odom_estimate;
        tf2::fromMsg( msg->estimate, odom_estimate );
        
        // Create timestamp from header
        rclcpp::Time stamp( msg->header.stamp );
        
        // Add keyframe to graph database
        // Note: In server backend mode, we receive already-processed keyframes
        // so we add them directly without further filtering
        graph_database_->add_odom_keyframe( stamp, odom_estimate, msg->accum_distance, cloud, 
                                           std::make_shared<sensor_msgs::msg::PointCloud2>( msg->cloud ) );
        
        RCLCPP_INFO( get_logger(), "Added keyframe to graph: robot=%s, cloud_size=%zu, accum_dist=%.2f",
                    msg->robot_name.c_str(), cloud->size(), msg->accum_distance );
    }
    ...
    void optimization_timer_callback()
    {
        ...
        if( keyframes.empty() || prev_robot_keyframe == nullptr || prev_robot_keyframe->node == nullptr ) {
            RCLCPP_DEBUG( get_logger(), "No keyframes to optimize yet, skipping optimization" );
            optimization_timer_->reset();
            return;
        }
        ...
        int num_iterations = graph_slam_->optimize( get_parameter( "g2o_solver_num_iterations" ).as_int(), get_parameter( "g2o_verbose" ).as_bool() );
        end = std::chrono::high_resolution_clock::now();
        graph_optimization_times_.push_back( std::chrono::duration_cast<std::chrono::microseconds>( end - start ).count() );

        // Check if optimization was successful
        if( num_iterations < 0 ) {
            RCLCPP_WARN( get_logger(), "Graph optimization failed, skipping pose update" );
            optimization_timer_->reset();
            return;
        }
        ...
        if( !enable_keyframe_event_reception_ && odom2map_pub_->get_subscription_count() ) {
...
private:
    ...
    // B-plan: Keyframe event publisher (for server upload, robot mode)
    rclcpp::Publisher<mrg_slam_msgs::msg::KeyframeEvent>::SharedPtr keyframe_event_pub_;
    // B-plan: Keyframe upload subscriber (for server backend mode)
    rclcpp::Subscription<mrg_slam_msgs::msg::KeyframeEvent>::SharedPtr keyframe_upload_sub_;
    bool enable_keyframe_event_reception_;  // Server backend mode flag
