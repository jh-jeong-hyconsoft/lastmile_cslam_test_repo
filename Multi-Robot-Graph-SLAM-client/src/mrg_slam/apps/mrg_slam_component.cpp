...
public:
    ...
    virtual void onInit()
    {
        ...
        publish_slam_pose_with_covariance_ = get_parameter( "publish_slam_pose_with_covariance" ).as_bool();
        slam_pose_cov_base_pos_            = get_parameter( "slam_pose_cov_base_pos" ).as_double();
        slam_pose_cov_base_rot_            = get_parameter( "slam_pose_cov_base_rot" ).as_double();
        slam_pose_cov_fitness_ref_         = get_parameter( "slam_pose_cov_fitness_ref" ).as_double();
        slam_pose_cov_fitness_scale_       = get_parameter( "slam_pose_cov_fitness_scale" ).as_double();
        ...
        if( publish_slam_pose_with_covariance_ ) {
            ...
            scan_matching_status_sub_
                = create_subscription<mrg_slam_msgs::msg::ScanMatchingStatus>( "scan_matching_odometry/status", rclcpp::QoS( 10 ),
                                                                               std::bind( &MrgSlamComponent::scan_matching_status_callback,
                                                                                          this, std::placeholders::_1 ) );
        }
...
private:
    void initialize_params()
    {
        ...
        // SLAM pose covariance publishing
        declare_parameter<bool>( "publish_slam_pose_with_covariance", true );
        declare_parameter<double>( "slam_pose_cov_base_pos", 0.009 );      // 3cm GNSS var (0.0009) * 10
    ...
    void publish_slam_pose( mrg_slam::KeyFrame::ConstPtr kf )
    {
        ...
        publish_slam_pose_with_covariance( kf );
    }

    void publish_slam_pose_with_covariance( mrg_slam::KeyFrame::ConstPtr kf )
    {
        if( !publish_slam_pose_with_covariance_ || !slam_pose_cov_pub_ ) {
            return;
        }
        ...
        if( slam_pose_cov_fitness_ref_ > 0.0 ) {
            double ratio = latest_matching_error_ / slam_pose_cov_fitness_ref_;
            if( ratio < 0.0 ) {
                ratio = 0.0;
            }
            double capped = std::min( ratio, slam_pose_cov_fitness_scale_ );
            scale += capped;
        }

        double pos_var = slam_pose_cov_base_pos_ * scale;
        double rot_var = slam_pose_cov_base_rot_ * scale;
        ...
        slam_pose_cov_pub_->publish( cov_msg );
    }

    void scan_matching_status_callback( const mrg_slam_msgs::msg::ScanMatchingStatus::SharedPtr msg )
    {
        latest_matching_error_        = msg->matching_error;
        latest_matching_status_stamp_ = rclcpp::Time( msg->header.stamp );
    }
...
private:
    ...
    rclcpp::Publisher<geometry_msgs::msg::PoseWithCovarianceStamped>::SharedPtr                  slam_pose_cov_pub_;
    ...
    rclcpp::Subscription<mrg_slam_msgs::msg::ScanMatchingStatus>::SharedPtr scan_matching_status_sub_;
    ...
    bool    publish_slam_pose_with_covariance_{true};
    double  slam_pose_cov_base_pos_{0.0};
    double  slam_pose_cov_base_rot_{0.0};
    double  slam_pose_cov_fitness_ref_{0.0};
    double  slam_pose_cov_fitness_scale_{0.0};
    double  latest_matching_error_ = 0.0;
    rclcpp::Time latest_matching_status_stamp_;
