...
private:
    ...
    /**
     * @brief lookup odom -> cloud transform and return as matrix, with fallback to latest TF
     */
    bool lookup_odom_to_cloud( const rclcpp::Time& stamp, const std::string& cloud_frame, Eigen::Matrix4f& out_pose )
    {
    ...
    Eigen::Matrix4f matching( const rclcpp::Time& stamp, const pcl::PointCloud<PointT>::ConstPtr& cloud )
    {
        if( !keyframe_cloud_ ) {
            ...
            // Align the initial keyframe pose to the external odom TF so we start in the RL odom frame
            keyframe_pose_.setIdentity();
            if( !lookup_odom_to_cloud( stamp, cloud->header.frame_id, keyframe_pose_ ) ) {
                RCLCPP_WARN( get_logger(), "Init keyframe pose fallback to identity" );
            }
        ...
        if( get_parameter( "enable_imu_frontend" ).as_bool() ) {
            ...
            } else if( tf_buffer_->canTransform( cloud->header.frame_id, rclcpp::Time( 0 ), cloud->header.frame_id, prev_time_,
            ...
            } else if( tf_buffer_->canTransform( cloud->header.frame_id, rclcpp::Time( 0 ), cloud->header.frame_id, rclcpp::Time( 0 ),
                                                 robot_odom_frame_id, rclcpp::Duration( 0, 0 ) ) ) {
                // 마지막 수신 TF라도 써서 현재 로봇이 내보낸 최신 odom->lidar를 초기 추정에 사용
                try {
                    transform = tf_buffer_->lookupTransform( cloud->header.frame_id, rclcpp::Time( 0 ), cloud->header.frame_id,
                                                             rclcpp::Time( 0 ), robot_odom_frame_id );
                    RCLCPP_WARN( get_logger(), "Using latest available odom->lidar TF for init guess (no stamped TF available)" );
                } catch( const tf2::TransformException& ex ) {
                    RCLCPP_WARN( get_logger(),
                                 "Could not look up latest transform with target frame %s, source frame %s, fixed frame %s: %s",
                                 cloud->header.frame_id.c_str(), cloud->header.frame_id.c_str(), robot_odom_frame_id.c_str(), ex.what() );
                }
            }
        ...
        Eigen::Matrix4f icp_odom  = keyframe_pose_ * trans;  // pure ICP accumulation (for keyframes/graph)

        // RL odom backbone + ICP correction for published odom
        Eigen::Matrix4f publish_odom = icp_odom;
        Eigen::Matrix4f corr         = trans;
        if( !msf_source.empty() ) {
            corr = msf_delta.matrix().inverse() * trans;
        }
        Eigen::Matrix4f rl_pose;
        if( lookup_odom_to_cloud( stamp, cloud->header.frame_id, rl_pose ) ) {
            publish_odom = rl_pose * corr;
        }
        ...
        if( get_parameter( "enable_tf_broadcast" ).as_bool() ) {
            std::string odom_frame_id      = get_parameter( "odom_frame_id" ).as_string();
        ...
        if( aligned_points_pub_->get_subscription_count() > 0 ) {
            std::string odom_frame_id = get_parameter( "odom_frame_id" ).as_string();
            pcl::transformPointCloud( *cloud, *aligned, publish_odom );
    ...
    void publish_odometry( const rclcpp::Time& stamp, const std::string& base_frame_id, const Eigen::Matrix4f& pose )
    {
        ...
        // broadcast the transform over tf (optional, controlled by parameter)
        if( get_parameter( "enable_tf_broadcast" ).as_bool() ) {
